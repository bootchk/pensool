'''
A control in the background.  The root control, in charge when no other controls.
'''
'''
...
class BackgroundControl(gui.control.GuiControl):
  '''
  A singleton special control that manages background,
  ...
  def set_background_bounds(self):
    ...
    '''
    self.bounds.from_rect(config.viewport.da.allocation)
  ...
  def pick_cb(self, point):
    ...
    picked_morph = config.scheme.model.pick(context, point)
    if picked_morph:
    ...
    self._open_menu(point, config.scheme.model, controlinstances.document_handle_menu)
    return True
  ...
  def button_press_right(self, event):
    ...
    '''
    self._open_menu(event, config.scheme.model, controlinstances.document_menu)
  
  
  #@dump_event
  def scroll_up(self, event):
    ''' Scroll wheel event in background is zoom op on *document*. '''
    config.scheme.zoom(True, event) # zoom in
  ...
  def control_key_release(self, event):
    ...
    '''
    self._open_menu(self.pointer_DCS, config.scheme.model, controlinstances.handle_menu)
  ...
  def start_drag(self, event):
    ...
    gui.manager.drop.dropmgr.begin(event, controlee=config.scheme.model, control=self)
